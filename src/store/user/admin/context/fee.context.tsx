...
const FeeProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const handleSubmit = async (
    ...
    console.log(newData);
    const data = await apiPOST("/api/admin/fee/fee-create", newData);
    // console.log(data);
    if (data.success) {
  ...
  const feeList = async () => {
    try {
      const res = await fetch("/api/admin/fee/admin-view-fee-list");
  ...
  const feeDetails = async (id: string) => {
    try {
      ...
      const res = await fetch(`/api/admin/fee/admin-view-fee-detail/${id}`);
